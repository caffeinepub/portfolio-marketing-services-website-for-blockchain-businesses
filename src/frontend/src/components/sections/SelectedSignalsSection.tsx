...
const signals = [
  {
    ...
    showPreview: false,
    showXEmbed: true,
    xPostUrl: 'https://x.com/AvailProject/status/1881349869084074419'
  },
  {
          ...
          {signals.map((signal, index) => (
            <Card key={index} className="border-accent/20 hover:border-accent/40 transition-colors">
              ...
              <CardContent className="space-y-4">
                {/* X Post Embed - only for AVAIL signal */}
                {signal.showXEmbed && signal.xPostUrl && (
                  <div className="my-4">
                    <XPostEmbed tweetUrl={signal.xPostUrl} />
                  </div>
                )}
